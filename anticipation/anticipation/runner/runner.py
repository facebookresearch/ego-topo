...
from mmcv.runner.hooks import (CheckpointHook, Hook, IterTimerHook, LrUpdaterHook,
    ...
    def update(self, vars, count=1):
        ...
        for key, var in vars.items():
            ...
            if 'AP' not in key: 
                ...
                # print ('UPDTE: logging AP')
                # print ('Adding', var[0].shape, var[1].shape)
                # print ('Now at', self.apmeter.scores.shape)
                if key not in self.apmeter:
                    self.apmeter[key] = tnt.meter.APMeter()

                self.apmeter[key].add(*var)
                self.val_history[key] = True
    ...
    def average(self, n=0):
        ...
        for key in self.val_history:
            ...
            if 'AP' not in key:
                ...
                # print ('AVG: logging AP')
                # print ('SHAPE:', self.apmeter.scores.shape)
                # print(key, self.apmeter[key].value())
                self.output[key] = float(self.apmeter[key].value().mean())
