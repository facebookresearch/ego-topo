...
def get_visit_feature(trunk_features, start, stop, dim, test_mode=True):
    feats = []
...
class EpicNodeDataset(Dataset):
    def __init__(self, graph_root, fb, ann_file=None, add_verb=False, add_noun=False, fb_dim=2048, max_length=15, test_mode=True):
    ...
    def build_dataset(self, graph_root, videos, out_file):
        ...
        for vid, graph_history in graphs.items():
            ...
            for node in G.nodes:
                ...
                #-----------------------------------#
                # some visits may have no features?
                _visits = []
                for visit in visits:
                    start, stop = visit['start'][1], visit['stop'][1]
                    feats = []
                    for fid in range(start - 31, stop - 31):
                        if fid in self.fb[vid].keys():
                            feats.append(to_tensor(self.fb[vid][fid]))
                    if len(feats)==0:
                        continue
    ...
    def __getitem__(self, idx):
        ...
        feats = [get_visit_feature(self.fb[vid], v['start'][1], v['stop'][1], dim=self.fb_dim, test_mode=self.test_mode) for v in visits]
